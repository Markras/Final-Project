...
The major part of my project is data cleaning and treatment in order to build easily evaluable datasets containing information we need. Moreover, basic tools such as ggplot visualization, statistical t-test as well as linear model will be used.

For the sake of space and clarity the full code will be accessible here: '' 
...
And what about the perception of reviews? Let us verify whether the same logic applies to votes funny and votes useful. Remember that votes concern not the service itself, but the reviews!

```{r, echo=FALSE,warning=FALSE, size=8}
print("average of votes$funny")
means_funny <- (means[order(means[3,])])
means_funny[3,]
print("average of votes$useful")
means_useful <- (means[order(means[4,])])
means_useful[4,]

```
Now this is interesting. Automotive services' reviews are not funny at all, while Local Flavor and church reviews seem to be hilarious. Mark that Real Estate reviews are very funny, even though the services itself tend too be poorly judged. (So there is no correlation between funny review and high notes). Curiously, Real Estate reviews are very funny, but not really judged as useful! 

However, this analysis is deeply flawed. We cannot presume that the number of readers is proportional to the number of reviews, and we don't know how frequently reviews are upvoted. Happily there are additional data inside the user dataset contanining information on how much votes and compliments a reviewer gets. Using this data we will try to evaluate the impact of categories on the vote and compliments obtained by reviewers.
...
g <- ggplot(newUser, aes(Religious_Organizations, average_stars)) + geom_point(colour="grey50") + geom_smooth(method="lm") + xlab("RO Reviews")
g

g <- ggplot(newUser, aes(Real_Estate, average_stars)) + geom_point(colour="grey50") + geom_smooth(method="lm")+ xlab("Real Estate Reviews")
g
...
Even though their number is comparatively low, our extreme cases seem to be confirmed. More a given user writes reviews of religious institutions, the higher is the average of stars he attributes. The opposite is true for Real Estate reviews.

What about usefulness of reviews? 

```{r, echo=FALSE, fig.width=2.1, fig.height=2.1, warning=FALSE, size=8}
colnames(UserList)[c(7:28)] <- Cat_names
newUser <- UserList
g <- ggplot(newUser, aes(Religious_Organizations, votes$useful)) 
g + geom_point(colour="grey50") +geom_smooth(method="lm")+ xlab("Religious Org. Reviews") + ylim(0, 20000)

g <- ggplot(newUser, aes(Automotive, votes$useful)) 
g + geom_point(colour="grey50")+ geom_smooth(method="lm") + xlab("Automotive Reviews") + ylim(0, 20000)
...
Here the pattern seems to contradict our previous findings. The total number of votes will get higher anyway with the growing number of reviews. We have to quantify this relation with linear model function to get definite answers then.

First, let us have a look on the impact of business categories on the votes qualified as useful.
```{r}
...
Our results seem to confirm our initial hypothesis. One's profile statistics not only reflect personal qualities of a given person, but also the subject of his inquiries. However the way it happens does not follow any clear pattern. We see however for sure that someone devoted to evaluating first of all Real Estate business will have considerably fewer compliments than someone reviewing Hotels with an equal involvement. Reviewers will have fewer votes "useful" if they review Financial Services, than if they review Hotels or Religious Organizations. Exactly the same applies to votes funny.
In other words, it is considerably easier to obtain positive feedback when evaluating certain categories. 

## Discussion
Many discussion-opening conclusions are possible. First of all, this kind of differences may be closely related to the fact, that much fewer people visit Yelp to read about Financial Institutions or Real Estate, than about Hotels. Yelp is centered around some activities and people are more likely to interact with certain kinds of content. However linear model does not follow simply number of reviews in a given category. Restaurants are by far most reviewed, and they are in the middle of both rankings.
